...
- NO hardcoded maps — uses HuntingZoneManager for dynamic zone recommendations
...
class CrossHorizonSynergy:
    """Detects and resolves conflicts between short-term and long-term goals."""
    def __init__(self):
        self._lock = RLock()
...
class GoalDecomposer:
    """Decomposes high-level goals into actionable sub-goals across horizons.
    Thread-safe: all state protected by RLock. Uses HZM for dynamic zone recs.
    """

    HORIZON_TTL = {
        GoalHorizon.tactical: 30, GoalHorizon.short_term: 300,
    ...
    def set_hunting_zone_manager(self, hzm) -> None:
        self._hzm = hzm

    def decompose(self, bot_id: str, assessment=None, goal_stack=None, selected=None):
        if selected is None:
            return None
        with self._lock:
            ...
            if category_str in ("survival",):
                ...
                target_map = self._recommend_zone(bot_id, objective)
                ...
                deps["interact"] = ["find_npc"]
                deps["complete"] = ["interact"]
            else:
    ...
    def _recommend_zone(self, bot_id: str, objective: str) -> str:
        if self._hzm is not None:
            try:
                zones = self._hzm.recommend_zone(bot_level=1, goal="leveling")
                if zones:
                    return zones[0].map_name
            except Exception:
                pass
        return "prt_fild08"
