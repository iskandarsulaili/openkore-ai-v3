...
class HeuristicService:
    ...
    def _assess_impl(self, signals: dict[str, Any], bot_id_override: str | None = None) -> HeuristicAssessment:
        ...
        if _audit_is_hunting:
            ...
            if _jc_h_eligible:
                # ── CONSCIOUS-DECIDED SURVIVAL STRATEGY GATE (RULE.md) ──
                # progression.py's _handle_job_change honors the DB-backed
                # survival_strategy=level_up_first (defer job change, farm a safe
                # map first). This SEPARATE emitter (heuristic_service) did NOT,
                # so it re-emitted `move <guild>` every 60s and fought the
                # conscious decision — the bot oscillated alberta_in <-> farm and
                # never reached the guild nor farmed (EXP froze). Mirror the gate
                # here so BOTH emitters defer consistently.
                _jc_h_surv = ""
                try:
                    from ai_sidecar.server_adaptation import get_server_solutions_store
                    _jc_h_surv_raw = get_server_solutions_store().get("survival_strategy", None)
                    if isinstance(_jc_h_surv_raw, dict):
                        _jc_h_surv = str(_jc_h_surv_raw.get("strategy", "") or "").strip().lower()
                    elif isinstance(_jc_h_surv_raw, str):
            ...
            # ── BROKE-SELL TRIGGER (2026-09-14) ── (OUTSIDE the potion gate)
            # A BROKE bot (zeny<500) carrying salable junk (weight>5%) MUST return
            # to town and sell to fund progression (job change / gear / potions).
            # The 60s cooldown + the fact that selling drops weight below 5% prevent
            # town-hopping (after a sale the trigger can't re-fire until the bot
            # re-accumulates loot). Robust to the fragile job_level signal: we do NOT
            # gate on eligibility here — converting junk to zeny is sound play for ANY
            # broke bot, and the in-town state machine already decides SELL at weight>5%.
            _audit_zeny = int(signals.get("zeny", 0) or 0)
            _audit_weight2 = float(signals.get("weight_ratio", 0.0) or 0.0)
            if (_audit_zeny < 500) and (_audit_weight2 > 0.05):
                ...
                if _audit_now2 - _audit_last_return2 > 60:
                    ...
                    actions.append(HeuristicAction(
                        kind="command", command="move prontera",
                        confidence=0.99, domain="economy",
                        reason="Broke (zeny<500) + carrying junk — return to town to sell and fund progression",
                    ))
        ...
        if state == "JOB_CHANGE":
            ...
            if map_name != _jc_npc_map:
                ...
                # LATCH (2026-09-02): emit at most once per 10s per bot — the
                # JOB_CHANGE state handler runs EVERY cycle and re-emitting
                ...
                # guild, NOT chase monsters — but it MUST defend itself when
                # attacked (attackAuto 0 = never fight back -> the low-HP novice
                # just takes hits from swarming field mobs and dies en route,
