...
- [2026-08-27] 6.4 AGNOSTIC AUDIT — removed REMAINING hardcoded map literals in LIVE heuristic_service._assess_impl decision path: (a) farm-target ladder (prt_fild05/pay_dun00/orcsdun01) → learned server_solutions farm_map → reachable_hunting_maps → adaptive get_best_map; (b) `move prontera` town fallback → learned safe_town → hunting-map-derived town → true-last-resort prontera; (c) _FARMS_OK hardcoded set → portal-graph hunting-map resolver; (d) raw weapon recompute → latched _has_coldstart_weapon. Fixed tuple-unpack bugs (_reach[0][0], _mk_get_hunting_maps(_bl_c) -> set comprehension). Legacy autonomy/domains/{economy,progression}.py literals are OBSERVE-ONLY (never executed, converted to kind=log). Verified onboarding_service move always carries DB-optimized target_map (prt_fild01 fallback unreachable).
