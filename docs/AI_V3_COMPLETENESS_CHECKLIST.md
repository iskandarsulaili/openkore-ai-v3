...
- [2026-09-01] ROOT CAUSE FIXED (map-login 0x0436 length): the RUNNING map-server binary (rebuilt 08:08 today, "(deleted)" inode) expects 0x0436 = **23 bytes** — verified live: "expected packet length 23, but only 21 bytes remaining" when 19 sent, "unknown connect packet 0x0436(length:25)" when 23+2. The learned 23-byte mapLoginLayout (account@2 char@6 login1@10 login2@14 tick@18 sex@22) is CORRECT for the live binary. The source clif_packetdb.hpp says 19 but the running binary was built from a 23-byte source state. FIX: (a) restored the 23-byte mapLoginLayout in testbotA config; (b) DirectConnection.pm SELF-HEAL — a learned layout is now invalidated after 3 consecutive map-login failures (cleared + rotation resumes) so a server-side length change self-heals instead of looping forever. VERIFIED LIVE: TestBotA logged in, killing Little Poring (Dmg 55, "Monster Died"), base_exp 803->1919, BAL kill events firing. [UNCOMMITTED — this batch]
- [2026-09-01] FLEET CLEANUP: disabled kicapmasin10/4/5 profiles (wrong passwords -> IP-ban loop) + testbotB/C (share account testbot99, no mapLoginLayout, collide with testbotA) into .bot_profiles_disabled/. Only testbotA (L11 farmer) runs. [UNCOMMITTED — this batch]
- [2026-09-01] LIVE PROGRESS VERIFIED (sustained): TestBotA base_level 11->12, base_exp 803->5505+ climbing, killing Little Poring/Lunatic continuously (BAL kill events firing), BotDetection confidence 0.10-0.28 (Human) mostly. Three-tier mind LIVE: all 5 brains (reflex/subconscious_ml/goal_decomposer/memory/strategy) record kill rewards for testbot99 in brain_rewards. Sidecar actor-ingest WORKING (v2_actors_ingested 200 OK, actor_delta_ingested at 09:18). DQN subconscious: 63 experiences, training_steps=0 (needs 64=batch_size to train — 1 short, will cross). [VERIFIED — this batch]
