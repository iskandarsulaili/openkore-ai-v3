...
- [2026-08-26] BATCH 3 (inventory) DONE + LIVE-VERIFIED: bridge dead `$char->{inventory}` key → real `$char->inventory()` via getItems()/get(binID); inventory_items digest (id/name/qty/type/equipped) + has_weapon_in_inventory in snapshot payload; sidecar reads top-level; InventoryItemDigest contract + type field; item_id stringified; object-branch signals. Commits d9117f163, 91e7e9d25, [type], [signals]. VERIFIED: newest snapshot has_weapon=True, 9 items (Novice Potion 569 x300, knife, suit).
- [2026-08-26] BATCH 2 partial (agnostic lockMap): get_best_map FARM-GATE (only *_fild/*_dun — excludes int_land island pollution). _audit_zeny UnboundLocalError fixed (potions-present path). Committed. Routing now must pick a valid farm map for a weapon-holding bot.
- [2026-08-26] NEW BUGS found: (3.1b) PartyCoordinator 'get_bot' attr missing; (RC3b) get_best_map returned int_land (fixed farm-gate). LLM advisory IS firing (provider_route_decided, _post_json every ~10s).
